...
async function checkExamSchedules() {
    try {
        ...
        console.log('Cấu hình thi V30:', 
            v30Constraint ? `${v30Constraint.min_days_before_exam} ngày trước, ${v30Constraint.exam_duration_hours} giờ, ${v30Constraint.exam_phases || 1} giai đoạn` : 'Không có');
        console.log('Cấu hình thi V31:', 
            v31Constraint ? `${v31Constraint.min_days_before_exam} ngày trước, ${v31Constraint.exam_duration_hours} giờ, ${v31Constraint.exam_phases || 1} giai đoạn` : 'Không có');
        ...
        v30ExamSchedules.forEach((exam, index) => {
            ...
            console.log(`  ${status} V30 - Thi ${index + 1}: Tuần ${exam.week_number}, Ngày ${exam.day_of_week}, Lớp ${classInfo}, Giai đoạn ${exam.exam_phase || 1}/${exam.total_phases || 1}`);
        });
        ...
        v31ExamSchedules.forEach((exam, index) => {
            ...
            const classInfo = exam.class ? exam.class.toString() : 'unknown';
            console.log(`  ${status} V31 - Thi ${index + 1}: Tuần ${exam.week_number}, Ngày ${exam.day_of_week}, Lớp ${classInfo}, Giai đoạn ${exam.exam_phase || 1}/${exam.total_phases || 1}`);
        });
        ...
        if (openingCeremonySchedules.length === 0) {
            ...
            openingCeremonySchedules.forEach((ceremony, index) => {
                ...
                const isCorrectDate = ceremonyDate && 
                    (ceremonyDate.getDate() === 15 || ceremonyDate.getDate() === 16) && 
                    ceremonyDate.getMonth() === 8; // Tháng 9 (0-indexed)
                
                const status = isCorrectDate ? '✅' : '❌';
                console.log(`  ${status} Khai giảng ${index + 1}: ${ceremonyDate ? ceremonyDate.toLocaleDateString() : 'Không có ngày'}, Tuần ${ceremony.week_number}, Ngày ${ceremony.day_of_week}`);
                
                if (!isCorrectDate && ceremonyDate) {
                    console.log(`     ⚠️ Ngày khai giảng không đúng! Dự kiến: 15-16/09, thực tế: ${ceremonyDate.getDate()}/${ceremonyDate.getMonth() + 1}`);
                }
            });
        }
