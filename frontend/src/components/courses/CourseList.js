...
const CourseList = () => {
  ...
  const filteredCourses = courses.filter(course => {
    if (course.departmentId !== selectedDepartment && course.department_id !== selectedDepartment) {
      return false;
    }
    
    if (courseFilter === 'main' && course.parent_course_id !== null) {
      return false;
    }
    
    if (courseFilter === 'sub' && course.parent_course_id === null) {
      return false;
    }
  ...
  // Helper function to get actual department ID from course object
  const getDepartmentId = (course) => {
    return course.departmentId || course.department_id;
  };
  ...
  const getParentCourseName = (parentId) => {
    const parentCourse = courses.find(c => c.id === parentId);
                      ...
                      <Link to={`/courses/${course.id}`} style={{ textDecoration: 'none', color: 'inherit' }}>
                        ...
                        {course.parent_course_id && (
                          <Typography variant="caption" color="text.secondary">
                            Thuộc môn: {getParentCourseName(course.parent_course_id)}
                          </Typography>
                        )}
                      </Link>
                    </TableCell>
              ...
              <Grid item xs={12} sm={6} md={4} key={course.id}>
                <Card sx={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
                  <CardContent sx={{ flexGrow: 1 }}>
                    ...
                    {course.parent_course_id && (
                      <Typography variant="body2" color="text.secondary" sx={{ mb: 1 }}>
                        Thuộc môn: <strong>{getParentCourseName(course.parent_course_id)}</strong>
                      </Typography>
                    )}
